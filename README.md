...
它不是粗暴删除所有空格，而是中文语境下的智能空格规范化工具：会移除中文与英文、数字、中文之间不自然的空格，同时保留英文短语内部空格，例如 `OpenAI API`、`Visual Studio Code`、`High Performance Web Server`。
...
- 支持标点附近空格清理。
- 支持实时预览、一键复制、一键清空、示例文本填充。
- 显示处理前后字符数和修改数量。
- 所有处理均在浏览器本地完成，不调用 API，不上传用户文本。

## 处理模式

- `conservative`：保守模式，只处理中文与英文、数字之间的空格。
- `standard`：标准模式，处理中文、中英、中数、标点附近空格。
...
本项目不包含后端、数据库或 API Key。所有文本处理逻辑都在浏览器本地执行，用户输入不会被上传到任何服务器。
